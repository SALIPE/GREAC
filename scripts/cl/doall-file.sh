...
REF_MEDIBEES_1=~/Desktop/datasets/medibees/data1/GCF_003254395.2_Amel_HAv3.1_genomic_chr1.fna
...
case $GROUPNAME in
    ...
    bees[0-9]*)
        ...
        ;;
    medibees[0-9]*)
        if [[ $GROUPNAME =~ ^medibees([0-9]+)$ ]]; then
            chr="${BASH_REMATCH[1]}"
            if (( chr >= 1 && chr <= 16 )); then

                REF_BEES="REF_MEDIBEES_${chr}"
                REF_TOTAL=$(eval echo \$$REF_BEES)

                SOURCE_VAR="MEDIBEES_${chr}"
                SOURCE=$(eval echo \$$SOURCE_VAR)
                echo "✅ Dataset MEDIBEES selecionado: $SOURCE"
            else
                echo "❌ Erro: Número fora do intervalo permitido (1–16): $chr"
                exit 1
            fi
