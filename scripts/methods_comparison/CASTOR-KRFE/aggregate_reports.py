...
# Metrics aggregated over the repetitions. k_length/n_k_mers only exist for
# CASTOR-KRFE, where the length of k is an outcome of the method, not a setting.
METRICS = ("precision", "recall", "f1_score", "k_length", "n_k_mers", "seconds")
...
def summarise(rows):
	...
	for key in sorted(groups, key = lambda key: (key[0], int(key[1]) if key[1].isdigit() else 0, key[2])):
		...
		for metric in METRICS:
			...
			entry[metric + "_mean"] = round(statistics.mean(values), 4) if values else ""
...
def main():
	...
	runs = len({row.get("run_log") for row in rows})
	print(str(len(rows)) + " report rows from " + str(runs) + " runs")
